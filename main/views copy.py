...
@api_view(['GET'])
def PoManagementCopy(request, search_refno):
    ...
    """
    result3=panda.raw_query(querystr,[search_refno,search_refno,search_refno])    
...
def PoSupplierCopy(request, search_refno):
    ...
    IF(in_kind=0,subtotal1,0) AS subtotal1,discount1 * -1 AS discount1,subtotal2,discount2,total,issuestamp,issuedby,postdatetime,postby,approvedby,a.laststamp,
    a.remark,
    ...
    CONCAT('PO Date ',DATE_FORMAT(podate,'%%Y-%%m'),'    Deliver on ',
    DATE_FORMAT(deliverdate,'%%Y-%%m-%%d'),'    Expiry ',DATE_FORMAT(expiry_date,'%%Y-%%m-%%d')) AS date_po,
    ...
    doc_name_reg,
    IF(in_kind=1,'Stock In-Kind Net','PO Net Amount') AS total_net_desc,approved_by,approved_at,
...
@api_view(['GET'])
def GrManagementCopy(request, search_refno):
    print(search_refno)
    ...
    """
    result2=panda.raw_query(querystr,[search_refno,search_refno,search_refno,search_refno,search_refno,search_refno,search_refno,search_refno,search_refno])    
    result["query2"] = result2
    ...
    WHERE b.refno=%s;
    """
    result3=panda.raw_query(querystr,[search_refno,search_refno,search_refno,search_refno,search_refno,search_refno,search_refno,search_refno,search_refno,search_refno])    
    result["query3"] = result3
...
@api_view(['GET'])
def GrSupplierCopy(request, search_refno):
    print(search_refno)
  ...
  SELECT a.*,b.* FROM
    ...
    result4=panda.raw_query(querystr,[search_refno,search_refno,search_refno,search_refno,search_refno,search_refno,search_refno,search_refno,search_refno,search_refno,search_refno])    
    result["query4"] = result4
    return Response(result, status=status.HTTP_200_OK)   

@api_view(['GET'])
def DiTaxInvoice(request, search_refno):
    print(search_refno)

    querystr = """
    SELECT a.refno,a.loc_group AS outlet,
    CONCAT(a.CODE,' - ',a.NAME) AS supplier,datefrom,dateto,freqofpayment,amountperpayment,totalpayment,lastpayment,firstpaymentdate,
    IF(e.created_by IS NULL,issuedby,IF(e.posted=0,issuedby,e.created_by)) AS issuedby,
    IF(e.created_at IS NULL,issuestamp,IF(e.posted=0,issuestamp,e.created_at)) AS issuestamp,
    a.docdate,
    a.remark AS remark,
    ...
    ROUND(paidamount+gst_tax_amt,2) AS total_aft_gst,
...
@api_view(['GET'])
def PromotionClaimInvoice(request, search_refno):
    print(search_refno)

    querystr = """
    SELECT c.*,a.loc_group,cardtype,itemcode,description,barcode,price_target,price_net,supplier,a.remark,
    bare_supplier_type,sys_average,trans_type,qtyclaim_manual,
    disc_value,sold_qty,datefrom,dateto,timefrom,timeto,a.posted_at,cancel_at,docno,promo_themes,
    /* amend on 2018-12-22 due to mydin generate inv from hq
    IF(a.issued_by_hq=1,CONCAT(a.refno,'-',a.loc_group),a.refno) AS refno,*/
    a.refno,
    convert_at,
    locdesc,warn_remark,bear_supplier,sold_bare_supplier,
    CONCAT(DATE_FORMAT(datefrom,'%%d/%%m/%%y %%a'),' to ',DATE_FORMAT(dateto,'%%d/%%m/%%y %%a')) AS promo_date,
    ROUND(sold_bare_supplier+(qtyclaim_manual*bear_supplier),2) AS sold_bear_sup,
    ROUND(gst_tax_amount,2) AS gst_tax_amount,a.gst_tax_rate,
    ROUND(ROUND(sold_bare_supplier+(qtyclaim_manual*bear_supplier),2)+gst_tax_amount,2) AS total_bear_gst,
    discount,
    ...
    bare_supplier_type,sys_average,trans_type,
    ...
    IF(d.promo_refno IS NULL,a.loc_group,d.taxinv_guid) AS taxinv_guid1
    ...
    bare_supplier_type,sys_average,a.trans_type,d.qtyclaim_manual,d.sold_bare_supplier,
...
@api_view(['GET'])
def GrSupplierGQV(request, search_refno):
    print(search_refno)
...
@api_view(['GET'])
def PurchaseReturnDN(request, search_refno):
    print(search_refno)
    ...
    result = panda.raw_query(querystr,[search_refno,search_refno,search_refno,search_refno,search_refno])
    result={"query1":result}
    ...
    result4=panda.raw_query(querystr,[search_refno,search_refno,search_refno,search_refno,search_refno,search_refno,search_refno])    
    result["query4"] = result4
    return Response(result, status=status.HTTP_200_OK)   

@api_view(['GET'])
def PurchaseReturnCN(request, search_refno):
    print(search_refno)
...
def report_SiSupplierCopy(request, search_refno):
    ...
    result4=panda.raw_query(querystr,[search_refno,search_refno,search_refno,search_refno,search_refno,search_refno,search_refno,search_refno,search_refno,search_refno,search_refno,search_refno,search_refno,search_refno])
    result["query4"] = result4
    return Response(result, status=status.HTTP_200_OK)
